...
class VideoCaptureView(QtWidgets.QGraphicsView, DetectDogs):
    ...
    def __init__(self, parent=None, video_path=None):
        ...
        self.breeds = []
        global label_path
        print( '=====================>>>>>>>>>>>>>>>>>'+'VideoCaptureView : 1. Get label')
        with open(label_path) as f:
            lines = f.readlines()
            for line in lines:
                self.breeds.append(line.replace('\n', ''))

        print( '=====================>>>>>>>>>>>>>>>>>'+'VideoCaptureView : 2. Set label color')
        color_palette = np.array(sns.color_palette(n_colors=len(self.breeds)))
        self.colors = color_palette * 255


        if video_path:
            print( '=====================>>>>>>>>>>>>>>>>>'+'VideoCaptureView : 3. Try open video')
            self.capture = cv2.VideoCapture(video_path)
        else:
            print( '=====================>>>>>>>>>>>>>>>>>'+'VideoCaptureView : 3. Try open webcam')
            self.capture = cv2.VideoCapture(0)

        if not self.capture.isOpened():
            print( '=====================>>>>>>>>>>>>>>>>>'+'VideoCaptureView : 3. Open Failed webcam!!!!')
            sys.exit()

        print( '=====================>>>>>>>>>>>>>>>>>'+'VideoCaptureView : 4. Video Scene Setting...')
    ...
    def make_sub_window(self):
        ...
        print( '=====================>>>>>>>>>>>>>>>>>'+'VideoCaptureView : ???????????')
        sub_window = SubWindow(breed_name=self.breed_label)
        sub_window.show()

    def keyPressEvent(self, event):
        print( '=====================>>>>>>>>>>>>>>>>>'+'VideoCaptureView : capture image! check save folder !')
